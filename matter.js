const {
  ...
  class MBall extends MEntity {
    ...
    tick(dt) {
      ...
      if (xt) {
        this.x =
          this.xv > 0 ? xt.hbox.x1 - this.w - epsilon : xt.hbox.x2 + epsilon;
      ...
      if (yt) {
        this.y =
          this.yv > 0 ? yt.hbox.y1 - this.h - epsilon : yt.hbox.y2 + epsilon;
  ...
  class MPlayer extends MEntity {
    ...
    tick(dt, events) {
      const canGroundPound =
        !this.grounded && !this.groundPounding && (this.airTime ?? 0) > 0.1;
  ...
  class MEnemy extends MEntity {
    ...
    tick(dt) {
      ...
      if (
        player &&
        this.hbox.collision(player.hbox) &&
        this.contactCooldown <= 0
      ) {
  ...
  class MWorld {
    ...
    build(room, bitmap, tileMap) {
      ...
      for (let row = 0; row < bitmap.length; row++) {
        ...
        for (let col = 0; col < line.length; col++) {
          ...
          const sprite =
            def.tileset[key8] ?? def.tileset[key4] ?? def.tileset["nnnn"];
          if (!sprite) {
            //remove later
            console.error(
    ...
    transportEntity(entity) {
      ...
      if (
        ...
        const dir =
          dc > 0 ? "right" : dc < 0 ? "left" : dr > 0 ? "bottom" : "top";
      ...
      super(x, y, 2, 4, (t, self) => {
        if (self.state === "lighting") {
          ...
          const frame = Math.min(
            frames.length - 1,
            Math.floor(
              (self.stateTime / MCheckpoint.LIGHT_UP_DURATION) * frames.length,
            ),
          );
          return frames[frame];
        }
  ...
  class MNPC extends MDecorative {
    ...
    _syncHTML() {
      ...
      const talking =
        this.dialogueIndex >= 0 && this.dialogueIndex < this.dialogue.length;
